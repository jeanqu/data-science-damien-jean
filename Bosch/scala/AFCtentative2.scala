...
s
 ...
 //Tentative de calcul du chi2 distance
 
 type oneLine = (Long, Array[Double], Array[Double])

def calculChiDistance(a1: Array[Double], a2: Array[Double], sumColonne: Array[Double]) = {
    var result = 0.toDouble
    val sa1 = a1.sum
    val sa2 = a2.sum
    for (l <- 0 to a1.length-1) 
    {
        result = result + Math.pow((a1(l)/sa1 - a2(l)/sa2), 2) / sumColonne(l)
    }
    Math.sqrt(result)
}

def initialize(line: oneLine, sumColonne: Array[Double]) = {
    var lineReturn = new Array[Double] (sumColonne.length)
     for (l <- 0 to lineReturn.length-1) 
    {
        lineReturn(l) = 0.toDouble
    }
    lineReturn(line._1.toInt) = calculChiDistance(line._2, line._3, sumColonne)
    lineReturn
}

def combine(acc: Array[Double], line: oneLine, sumColonne: Array[Double]) = {
    var new_acc = acc
    new_acc(line._1.toInt) = calculChiDistance(line._2, line._3, sumColonne)
    new_acc
}

def merge(acc1: Array[Double], acc2: Array[Double]) = {
    addTwoArray(acc1, acc2)   
}    

val ready_to_reduce = frequences.cartesian(frequences).map(line => (line._1._1, (line._2._1, line._1._2, line._2._2)))

val keyDeux = ready_to_reduce.combineByKey((line: oneLine) => initialize(line, bro_sumColonnes.value),
                                            (acc: Array[Double], line: oneLine) => combine(acc, line, bro_sumColonnes.value),
                                            (acc1:Array[Double], acc2: Array[Double]) => merge(acc1, acc2))
